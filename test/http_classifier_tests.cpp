//
// Created by klewy on 8/14/26.
//

#include <catch2/catch_all.hpp>
#include "common.hpp"

static unsigned char full_http_req[] = {0x45, 0x0, 0x1, 0xb5, 0xb3, 0xd, 0x40, 0x0, 0x40, 0x6, 0x56, 0xfd, 0xc0, 0xa8, 0x1, 0xa9, 0x68, 0x15, 0x4, 0xd2, 0xe5, 0x50, 0x0, 0x50, 0x99, 0xe, 0xe6, 0x92, 0xa1, 0x48, 0xe4, 0x55, 0x80, 0x18, 0x0, 0x3f, 0x30, 0xe0, 0x0, 0x0, 0x1, 0x1, 0x8, 0xa, 0x53, 0xfe, 0xf3, 0x17, 0x6, 0x44, 0x3b, 0xd7, 0x47, 0x45, 0x54, 0x20, 0x2f, 0x20, 0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x31, 0xd, 0xa, 0x48, 0x6f, 0x73, 0x74, 0x3a, 0x20, 0x68, 0x74, 0x74, 0x70, 0x66, 0x6f, 0x72, 0x65, 0x76, 0x65, 0x72, 0x2e, 0x63, 0x6f, 0x6d, 0xd, 0xa, 0x55, 0x73, 0x65, 0x72, 0x2d, 0x41, 0x67, 0x65, 0x6e, 0x74, 0x3a, 0x20, 0x4d, 0x6f, 0x7a, 0x69, 0x6c, 0x6c, 0x61, 0x2f, 0x35, 0x2e, 0x30, 0x20, 0x28, 0x58, 0x31, 0x31, 0x3b, 0x20, 0x4c, 0x69, 0x6e, 0x75, 0x78, 0x20, 0x78, 0x38, 0x36, 0x5f, 0x36, 0x34, 0x3b, 0x20, 0x72, 0x76, 0x3a, 0x31, 0x35, 0x33, 0x2e, 0x30, 0x29, 0x20, 0x47, 0x65, 0x63, 0x6b, 0x6f, 0x2f, 0x32, 0x30, 0x31, 0x30, 0x30, 0x31, 0x30, 0x31, 0x20, 0x46, 0x69, 0x72, 0x65, 0x66, 0x6f, 0x78, 0x2f, 0x31, 0x35, 0x33, 0x2e, 0x30, 0xd, 0xa, 0x41, 0x63, 0x63, 0x65, 0x70, 0x74, 0x3a, 0x20, 0x74, 0x65, 0x78, 0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x2c, 0x61, 0x70, 0x70, 0x6c, 0x69, 0x63, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0x2f, 0x78, 0x68, 0x74, 0x6d, 0x6c, 0x2b, 0x78, 0x6d, 0x6c, 0x2c, 0x61, 0x70, 0x70, 0x6c, 0x69, 0x63, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0x2f, 0x78, 0x6d, 0x6c, 0x3b, 0x71, 0x3d, 0x30, 0x2e, 0x39, 0x2c, 0x2a, 0x2f, 0x2a, 0x3b, 0x71, 0x3d, 0x30, 0x2e, 0x38, 0xd, 0xa, 0x41, 0x63, 0x63, 0x65, 0x70, 0x74, 0x2d, 0x4c, 0x61, 0x6e, 0x67, 0x75, 0x61, 0x67, 0x65, 0x3a, 0x20, 0x65, 0x6e, 0x2d, 0x55, 0x53, 0x2c, 0x65, 0x6e, 0x3b, 0x71, 0x3d, 0x30, 0x2e, 0x39, 0xd, 0xa, 0x41, 0x63, 0x63, 0x65, 0x70, 0x74, 0x2d, 0x45, 0x6e, 0x63, 0x6f, 0x64, 0x69, 0x6e, 0x67, 0x3a, 0x20, 0x67, 0x7a, 0x69, 0x70, 0x2c, 0x20, 0x64, 0x65, 0x66, 0x6c, 0x61, 0x74, 0x65, 0xd, 0xa, 0x43, 0x6f, 0x6e, 0x6e, 0x65, 0x63, 0x74, 0x69, 0x6f, 0x6e, 0x3a, 0x20, 0x6b, 0x65, 0x65, 0x70, 0x2d, 0x61, 0x6c, 0x69, 0x76, 0x65, 0xd, 0xa, 0x55, 0x70, 0x67, 0x72, 0x61, 0x64, 0x65, 0x2d, 0x49, 0x6e, 0x73, 0x65, 0x63, 0x75, 0x72, 0x65, 0x2d, 0x52, 0x65, 0x71, 0x75, 0x65, 0x73, 0x74, 0x73, 0x3a, 0x20, 0x31, 0xd, 0xa, 0x49, 0x66, 0x2d, 0x4d, 0x6f, 0x64, 0x69, 0x66, 0x69, 0x65, 0x64, 0x2d, 0x53, 0x69, 0x6e, 0x63, 0x65, 0x3a, 0x20, 0x57, 0x65, 0x64, 0x2c, 0x20, 0x30, 0x31, 0x20, 0x4a, 0x75, 0x6c, 0x20, 0x32, 0x30, 0x32, 0x36, 0x20, 0x31, 0x39, 0x3a, 0x33, 0x39, 0x3a, 0x31, 0x37, 0x20, 0x47, 0x4d, 0x54, 0xd, 0xa, 0x50, 0x72, 0x69, 0x6f, 0x72, 0x69, 0x74, 0x79, 0x3a, 0x20, 0x75, 0x3d, 0x30, 0x2c, 0x20, 0x69, 0xd, 0xa, 0xd, 0xa};
class HttpTestFixture
{
protected:
    ConnTracker tracker;
    Classifier classifier{tracker};

    constexpr static std::size_t ip_hdr_size = 20;
    constexpr static std::size_t tcp_hdr_size = 32;
    constexpr static std::size_t payload_offset = ip_hdr_size + tcp_hdr_size;

    const iphdr* ip{nullptr};
    const tcphdr* tcp{nullptr};
public:
    HttpTestFixture()
    {
        classifier.add(std::make_unique<HttpClassifier>());

        ip = reinterpret_cast<const iphdr*>(full_http_req);
        tcp = reinterpret_cast<const tcphdr*>(full_http_req + ip_hdr_size);
    }

    std::pair<std::vector<char>, std::vector<char>> split_at_pos(const int pos)
    {
        std::span<const char> full_payload(
        reinterpret_cast<const char*>(full_http_req) + payload_offset,
        sizeof(full_http_req) - payload_offset);

        // Split right after the first "\r\n" (end of "GET / HTTP/1.1\r\n"),
        // so the Host header lands entirely in the second fragment.
        const std::size_t split_at = pos; // offset within payload

        std::span<const char> first_fragment_payload = full_payload.subspan(0, split_at);
        std::span<const char> second_fragment_payload = full_payload.subspan(split_at);

        std::vector<char> first_packet = construct_packet_from(first_fragment_payload, ip, tcp);

        // Second fragment starts split_at bytes further into the stream,
        // so its sequence number must advance accordingly.
        std::vector<char> tcp_second_buf(tcp_hdr_size); // 32 bytes, full header + options
        std::memcpy(tcp_second_buf.data(), tcp, tcp_hdr_size);
        auto* tcp_second = reinterpret_cast<tcphdr*>(tcp_second_buf.data());
        tcp_second->seq = htonl(ntohl(tcp->seq) + static_cast<uint32_t>(split_at));

        std::vector<char> second_packet = construct_packet_from(second_fragment_payload, ip, tcp_second);

        return {first_packet, second_packet};
    }
};


TEST_CASE_METHOD(HttpTestFixture, "Full HTTP is classified correctly", "[http_classifier]")
{
    std::span<const char> packet{reinterpret_cast<const char*>(full_http_req), sizeof(full_http_req)};
    PacketView pkt = parse_packet_view(packet);

    tracker.track(pkt);
    auto res = classifier.classify(pkt);
    REQUIRE(res == ParseResult::SUCCESS);
    REQUIRE(pkt.payload_proto == L7Proto::HTTP);
    REQUIRE(pkt.is_payload_reasm == false);
}

TEST_CASE_METHOD(HttpTestFixture, "Split HTTP is classified correctly", "[http_classifier]")
{
    const auto [first_packet, second_packet] = split_at_pos(16);

    auto first_pkt = parse_packet_view(first_packet);
    auto second_pkt = parse_packet_view(second_packet);

    tracker.track(first_pkt);

    auto res = classifier.classify(first_pkt);
    REQUIRE(res == ParseResult::REASSEMBLING);
    REQUIRE(first_pkt.is_payload_reasm == true);

    tracker.track(second_pkt);

    res = classifier.classify(second_pkt);
    REQUIRE(res == ParseResult::SUCCESS);
    REQUIRE(second_pkt.payload_proto == L7Proto::HTTP);
    REQUIRE(second_pkt.is_payload_reasm == true);
}

TEST_CASE_METHOD(HttpTestFixture, "Split HTTP at headers end", "[http_classifier]")
{
    const auto [first_packet, second_packet] = split_at_pos(383);

    auto first_pkt = parse_packet_view(first_packet);
    auto second_pkt = parse_packet_view(second_packet);

    tracker.track(first_pkt);

    auto res = classifier.classify(first_pkt);
    REQUIRE(res == ParseResult::REASSEMBLING);
    REQUIRE(first_pkt.is_payload_reasm == true);

    tracker.track(second_pkt);
    res = classifier.classify(second_pkt);
    REQUIRE(res == ParseResult::SUCCESS);
    REQUIRE(second_pkt.is_payload_reasm == true);
    REQUIRE(second_pkt.payload_proto == L7Proto::HTTP);
}

TEST_CASE_METHOD(HttpTestFixture, "Split HTTP at Host Header", "[http_classifier]")
{
    const auto [first_packet, second_packet] = split_at_pos(18);

    auto first_pkt = parse_packet_view(first_packet);
    auto second_pkt = parse_packet_view(second_packet);

    tracker.track(first_pkt);

    auto res = classifier.classify(first_pkt);
    REQUIRE(res == ParseResult::REASSEMBLING);
    REQUIRE(first_pkt.is_payload_reasm == true);

    tracker.track(second_pkt);
    res = classifier.classify(second_pkt);
    REQUIRE(res == ParseResult::SUCCESS);
    REQUIRE(second_pkt.is_payload_reasm == true);
    REQUIRE(second_pkt.payload_proto == L7Proto::HTTP);
}

TEST_CASE_METHOD(HttpTestFixture, "Split HTTP at Host Header Value", "[http_classifier]")
{
    const auto [first_packet, second_packet] = split_at_pos(30);

    auto first_pkt = parse_packet_view(first_packet);
    auto second_pkt = parse_packet_view(second_packet);

    tracker.track(first_pkt);

    auto res = classifier.classify(first_pkt);
    REQUIRE(res == ParseResult::REASSEMBLING);
    REQUIRE(first_pkt.is_payload_reasm == true);

    tracker.track(second_pkt);
    res = classifier.classify(second_pkt);
    REQUIRE(res == ParseResult::SUCCESS);
    REQUIRE(second_pkt.is_payload_reasm == true);
    REQUIRE(second_pkt.payload_proto == L7Proto::HTTP);
}
